...
export function SearchPage({ viewMode }: Props) {
  ...
  return (
    <main className="max-w-app mx-auto px-8 py-8 flex flex-col gap-8">
      ...
      {state === "error_invalid" && (
        <ErrorState type="invalid_cnpj" onRetry={reset} />
      )}
      {state === "error_not_found" && (
        <ErrorState type="not_found" onRetry={reset} />
      )}
      {state === "error_api" && (
        <ErrorState type="api_failure" onRetry={reset} />
      )}

      {state === "idle" && (
        <div className="flex flex-col items-center gap-3 py-16 text-center animate-fade-in">
          <span className="text-6xl">⚡</span>
          <h2 className="font-display text-2xl text-neutral-300">
            Transforme dados em decisões
          </h2>
          <p className="text-neutral-500 max-w-sm text-sm">
            Digite um CNPJ acima para enriquecer o lead com score de prioridade,
            segmento de mercado e insights acionáveis.
          </p>
