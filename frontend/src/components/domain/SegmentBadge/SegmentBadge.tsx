const SEGMENT_MAP: Record<string, { icon: string; category: 'tech' | 'retail' | 'service' | 'industry' }> = {
...
function detectCategory(segment: string): { icon: string; category: 'tech' | 'retail' | 'service' | 'industry' } {
...
interface Props { segment: string; className?: string }

export function SegmentBadge({ segment, className = '' }: Props) {
  const { icon, category } = detectCategory(segment)
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xs text-xs font-semibold ${CATEGORY_STYLES[category]} ${className}`}>
      <span>{icon}</span>
      <span>{segment}</span>
    </span>
  )
