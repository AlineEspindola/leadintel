...
interface FormData { name: string; email: string; phone: string; cnpj: string }
interface Props { onSubmit: (data: FormData) => void; loading?: boolean }

export function LeadForm({ onSubmit, loading }: Props) {
  const [form, setForm] = useState<FormData>({ name: '', email: '', phone: '', cnpj: '' })
  ...
  const validate = (): boolean => {
    ...
    if (!form.cnpj || form.cnpj.replace(/\D/g,'').length !== 14) e.cnpj = 'CNPJ incompleto'
    setErrors(e)
  ...
  return (
    <div className="flex flex-col gap-4 bg-surface-raised border border-surface-border rounded-lg p-6">
      <div>
        ...
        <p className="text-sm text-neutral-500 mt-0.5">Informe os dados básicos para enriquecer automaticamente</p>
      </div>
