...
> = {
  ...
  typing: {
    border: "border-primary-400 shadow-glow-primary",
    icon: "✏️",
    message: "Digite todos os 14 dígitos",
  },
  valid: {
    border: "border-success-500 shadow-glow-success",
    icon: "✅",
    message: "CNPJ válido",
  },
  invalid: {
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="relative flex items-center">
        ...
        />
        <span
          className={`absolute right-9 text-base ${loading ? "animate-spin" : ""}`}
        >
          {cfg.icon}
        </span>
        {value && !loading && (
          <button
            onClick={clear}
            className="absolute right-3 text-neutral-500 hover:text-neutral-200 transition-colors text-xs"
          >
            ✕
          </button>
        )}
      ...
      {cfg.message && (
        <p
          className={`text-xs ${state === "invalid" ? "text-error-400" : state === "valid" ? "text-success-400" : "text-neutral-500"}`}
        >
          {cfg.message}
        </p>
      )}
