...
export function Input({ label, error, hint, suffix, className = '', ...rest }: InputProps) {
  return (
    <div className="flex flex-col gap-1">
      {label && <label className="text-xs font-bold uppercase tracking-widest text-neutral-400">{label}</label>}
      <div className="relative flex items-center">
        <input
          ...
          {...rest}
        />
        {suffix && <span className="absolute right-3 text-neutral-400">{suffix}</span>}
      </div>
...
interface BadgeProps { children: ReactNode; variant?: keyof typeof BADGE_VARIANTS; className?: string }

export function Badge({ children, variant = 'default', className = '' }: BadgeProps) {
  return (
    <span className={`inline-flex items-center gap-1 text-xs font-bold uppercase tracking-wide rounded-xs px-2 py-0.5 ${BADGE_VARIANTS[variant]} ${className}`}>
      {children}
    </span>
  )
}
...
// ─── Card ───────────────────────────────────────────────────────────────────
interface CardProps { children: ReactNode; className?: string; variant?: 'default' | 'raised' }

export function Card({ children, className = '', variant = 'default' }: CardProps) {
  return (
    <div className={`${variant === 'raised' ? 'bg-surface-raised' : 'bg-surface-ghost'} border border-surface-border rounded-lg p-6 ${className}`}>
      {children}
    </div>
  )
}
