...
export class ActiveCompanySpecification extends AbstractSpecification<Company> {
  isSatisfiedBy(c: Company) { return c.getStatus().isActive() }
}
export class NameSpecification extends AbstractSpecification<Company> {
  constructor(private name: string) { super() }
  isSatisfiedBy(c: Company) { return c.getLegalName().getValue().toLowerCase().includes(this.name.toLowerCase()) }
}
export class CNPJSpecification extends AbstractSpecification<Company> {
  constructor(private cnpj: string) { super() }
  isSatisfiedBy(c: Company) { return c.getCNPJ().getValue().includes(this.cnpj) }
}
export class StateSpecification extends AbstractSpecification<Company> {
  constructor(private uf: string) { super() }
  isSatisfiedBy(c: Company) { return c.getState().getValue() === this.uf }
}
export class SizeSpecification extends AbstractSpecification<Company> {
  constructor(private keyword: string) { super() }
  isSatisfiedBy(c: Company) { return c.getSize().getValue().toLowerCase().includes(this.keyword.toLowerCase()) }
}
